...
#--------------------------------------------------------------
#　ブロックが壊れたときに飛び散る破片（当たり判定を持たない）を追加
#--------------------------------------------------------------
class BrickParticle(Widget):

    '''レンガが壊れた瞬間に少しだけ飛び散る破片
    
    なぜ別クラスにするのか
    ・マリオ本体やステージ本体の責務と分けるため
    ・「見た目だけのオブジェクト」を独立管理するため
    
    入出力
    ・入力：初期位置（X,y）、初速（vx,vy)、生存時（life）
...
class StageWithBreakblock(Widget):
    '''
       step10
       横移動＋横衝突＋ジャンプ/重力/着地に加えて
       下からレンガにあったたらブロックを壊す処理を実装したステージ
       step11
       土管の外に出るまでは土管の上を水平移動 ↓
                           土管の外に出たら床に落下して左へ歩くを実装
       step12
       ブロック全消しでクリボーが即出現
    ...
    def __init__(self, **kwargs):
        ...
        なぜここで作る？
        画像の読み込みの失敗を起動直後きづけるようにするため
        出現は後でも部品の準備は初期化時に済ませたいから
        
        初期位置は「土管の右側」に寄せていく
        ここから左に出ていく間は土管の上端に高さを固定する'''
        self.goomba = Goomba(
    ...
    def spawn_brick_particles(self,brick:Image):
        '''
        レンガの位置を元に、簡易破片を複数生成する
        
        なぜ関数に分けるのか：
        ・update() を肥大化させないため
        ・「壊れた時の演出」という責務を独立させるため
        '''
        #レンガを４等分したような位置から、小片を飛ばすイメージ
        particle_settings=[
    ...
    def update(self, dt: float):
        ...
        #ゲームオーバー後は更新を止める
        #これによりマリオやクリボーが動かなくなる
        if self.game_over:
            return        
        
        #ステージ開始からの経過時間を加算
        #非同期処理ではなく既存のupdateループで時間を管理する
        ...
        #下からあったって壊す対象を一時的に集める箱
        #ループ中にself.bricksを直接削除すると不安定になるため
        #まずは候補だけを集めてループ後にまとめて消す        
        break_targets=[]
        ...
        for solid in solids:
            if self.mario.collide_widget(solid):
                ...
                if self.vy > 0 and solid in self.bricks:
                    ...
                    #それ以上めり込まないように上向き速度を止める
                    self.vy=0
                
                    #ループが終わった後に削除するために候補へ移動
                    break_targets.append(solid)
                
            
            if self.vy < 0 and self.mario.collide_widget(solid) and self.mario.y <= solid.y + solid.height:
                self.mario.y = solid.y + solid.height
                self.vy = 0
                self.on_ground = True
                    
            #実際にレンガを壊す処理
            #見た目から消し、当たり判定対象からも消す
        for brick in break_targets:
            if brick in self.bricks:
                self.spawn_brick_particles(brick)
...
class Step12GoombahitApp(App):
    ...
    def build(self):
        ...
        self.title = 'Pipe & Jump - Step12 Goomba hit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           Goomba Spawn'
